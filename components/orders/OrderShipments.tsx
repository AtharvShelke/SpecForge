...
const SHIPMENT_STATUS_MAP: Record<string, { label: string; cls: string }> = {
  ...
  PICKED_UP:   { label: 'Picked Up',    cls: 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' },
  IN_TRANSIT:  { label: 'In Transit',   cls: 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200' },
  OUT_FOR_DELIVERY: { label: 'Out for Delivery', cls: 'bg-violet-50 text-violet-700 ring-1 ring-violet-200' },
  DELIVERED:   { label: 'Delivered',     cls: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' },
  FAILED:      { label: 'Failed',        cls: 'bg-rose-50 text-rose-600 ring-1 ring-rose-200' },
  RETURNED:    { label: 'Returned',      cls: 'bg-stone-100 text-stone-600 ring-1 ring-stone-200' },
};
...
const CARRIER_URLS: Record<string, (tn: string) => string> = {
  ...
  'DTDC':       (tn) => `https://www.dtdc.in/tracking/shipment-tracking.asp?strCnno=${tn}`,
  ...
  if (sortedShipments.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 px-4">
        ...
        <p className="text-xs font-semibold text-stone-500 mb-0.5">No Shipments Yet</p>
        <p className="text-[11px] text-stone-400">Shipment details will appear here once the order is shipped.</p>
      </div>
    );
  }
  ...
  return (
    <div className="divide-y divide-stone-100">
      {sortedShipments.map((shipment) => {
        const statusCfg = SHIPMENT_STATUS_MAP[shipment.status.toUpperCase()] ?? {
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1">
              <div>
                ...
                <p className="text-[11px] text-stone-600 font-mono tabular-nums">
                  {new Date(shipment.createdAt).toLocaleDateString('en-IN', DATE_OPTS)}
                </p>
              </div>
              {shipment.estimatedDelivery && (
                <div>
                  ...
                  <p className="text-[11px] text-stone-600 font-mono tabular-nums">
                    {new Date(shipment.estimatedDelivery).toLocaleDateString('en-IN', DATE_OPTS)}
                  </p>
                </div>
              )}
            </div>
