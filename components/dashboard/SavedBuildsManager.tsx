...
const KpiCard = memo(
  ({
    ...
    >
      ...
      <p
        className={cn(
          "text-[9px] sm:text-[10px] mt-1 font-medium truncate opacity-90",
          subColor,
        )}
      >
        {sub}
...
const BuildRow = memo(
  ({ build, isExpanded, onToggle, onEdit, onDelete }: BuildRowProps) => {
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-4 shrink-0 ml-2">
            <div className="text-right">
              ...
              <p className="hidden sm:block text-[9px] text-stone-400 font-bold uppercase tracking-widest">
                Total
              </p>
            </div>
            <div
...
export default function SavedBuildsManager() {
  ...
  const {
    savedBuilds: builds,
    refreshSavedBuilds: fetchBuilds,
    updateSavedBuild,
    deleteSavedBuild,
    syncData,
    isLoading,
  ...
  const handleEditBuild = useCallback(
    (build: BuildGuide) => setEditingBuild(build),
    [],
  );
  const handleDeleteConfirm = useCallback(
    (id: string) => setDeleteConfirmId(id),
    [],
  );
  ...
  const handleEditTotalChange = useCallback(
        ...
        <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-md bg-white border-stone-200 rounded-2xl shadow-xl">
          ...
          {editingBuild && (
            <form onSubmit={handleSave} className="space-y-3 py-1">
              {EDIT_FIELDS.map(
                ({ label, field, placeholder, type, required }) => (
        ...
        <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-md bg-white border-stone-200 rounded-2xl shadow-xl">
          ...
          <DialogHeader className="pb-2">
            ...
            <DialogDescription className="text-xs text-stone-500 leading-relaxed pl-9">
              Are you sure you want to delete this saved build? This action
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 pt-2 flex-row">
