...
const BuilderFiltersTab = memo(function BuilderFiltersTab() {
  ...
  const overrideMap = useMemo(() => {
    ...
    overrides.forEach((o) =>
      map.set(`${o.specDefinitionId}:${o.categoryName}`, o),
    );
    return map;
  }, [overrides]);

  const getOverride = useCallback(
    (specId: string, category: string) => {
      ...
      try {
        ...
        setOverrides((prev) => {
          const idx = prev.findIndex(
            (o) =>
              o.specDefinitionId === specId && o.categoryName === categoryName,
          );
          if (idx >= 0) {
            const next = [...prev];
            next[idx] = result;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400">
            {filteredSpecs.length} filterable specs
          </span>
        </div>
        <select
      ...
      {filteredSpecs.length === 0 ? (
        <div className="text-center py-8 text-zinc-400 text-sm">
          No filterable specs found. Select a category or add spec definitions.
        </div>
      ) : (
        <div className="space-y-1.5">
          {filteredSpecs.map((spec) => {
            const catName =
              selectedCategory ||
              (spec as any).subCategory?.category?.name ||
              "";
            ...
            return (
              <div
                key={spec.id}
                className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isHidden ? "border-zinc-100 bg-zinc-50 opacity-60" : "border-zinc-100 bg-white"}`}
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-1.5 py-0.5 text-[9px] font-bold bg-zinc-100 text-zinc-500 rounded-full uppercase">
                      {spec.valueType}
                    </span>
                    {spec.isRange && (
                      <span className="px-1.5 py-0.5 text-[9px] font-bold bg-blue-100 text-blue-600 rounded-full uppercase">
                        Range
                      </span>
                    )}
                    {spec.filterGroup && (
                      <span className="px-1.5 py-0.5 text-[9px] font-bold bg-purple-100 text-purple-600 rounded-full uppercase">
                ...
                {catName && (
                  <div className="flex items-center gap-1.5 shrink-0">
                    <input
                      ...
                      onBlur={(e) => {
                        if (
                          e.target.value !== (override?.labelOverride || "")
                        ) {
                          handleSaveOverride(spec.id, catName, {
