...
function BuildCardCinematic({ build, index }: { build: any; index: number }) {
    ...
    return (
        <motion.div
            ...
            className="group relative flex flex-col h-[500px] rounded-3xl overflow-hidden cursor-pointer"
        >
            ...
            <div className="absolute inset-0 z-10 border border-white/10 rounded-3xl group-hover:border-indigo-500/50 transition-colors duration-500 pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col h-full p-6 lg:p-8">
                ...
                <div className="flex items-start justify-between mb-auto">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/10">
                        ...
                        <span className="text-[10px] font-bold text-white uppercase tracking-widest">Optimized</span>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white border border-white/10 group-hover:bg-indigo-600 transition-colors duration-300">
                ...
                <div className="mt-8 transform group-hover:-translate-y-2 transition-transform duration-500">
                    ...
                    {/* Compact Specs list */}
                    <div className="space-y-3 mb-6 opacity-80 group-hover:opacity-100 transition-opacity">
                        {components.map(cat => {
                    ...
                    <div className="pt-4 border-t border-white/10 flex items-end justify-between">
                        <div>
                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Total Build Cost</p>
...
export default function FeaturedBuildsSection({ builds }: { builds: any[] }) {
    ...
    return (
        <section className="py-24 bg-zinc-950 px-4 md:px-0" id="featured-builds">
            <Container>
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16">
                    ...
                    >
                        <Link
                            href="/build-guides"
                            className="group mt-6 md:mt-0 inline-flex items-center gap-2 px-6 py-3 rounded-full border border-white/20 text-sm font-bold text-white uppercase tracking-wider hover:bg-white hover:text-zinc-950 transition-all duration-300"
                        >
                            View All Builds
