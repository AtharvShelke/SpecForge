...
function buildProductValueMap(product: Product, specMap: Map<string, SpecMapEntry>) {
...
function matchesSelectedValues(productValues: Map<string, string[]>, filterId: string, selected: string[]) {
...
function matchesFilterSet(
  ...
  if (query.priceMin !== undefined && getProductPrice(product) < query.priceMin) return false;
  if (query.priceMax !== undefined && getProductPrice(product) > query.priceMax) return false;

  for (const [filterId, selectedValues] of selectedFilters.entries()) {
    if (filterId === skipFilterId) continue;
    if (!matchesSelectedValues(productValues, filterId, selectedValues)) return false;
  }

  return true;
}

function getFilterDependencies(filter: SpecDefinition) {
  const deps = new Map<string, Set<string>>();

  for (const dep of filter.childOptionDeps ?? []) {
    if (dep.childOptionId) continue;
    const parentValues = deps.get(dep.parentSpecId) ?? new Set<string>();
    if (dep.parentOption?.value) parentValues.add(dep.parentOption.value);
    deps.set(dep.parentSpecId, parentValues);
  }
...
function getOptionDependencies(option?: SpecOption | null) {
  const deps = new Map<string, Set<string>>();
...
export function buildDynamicCatalogResult(args: {
  ...
  for (const filter of query.filters) {
    if (filter.values.length > 0) selectedFilters.set(filter.specId, filter.values);
  }

  const productValueMaps = products.map((product) => ({
    product,
    values: buildProductValueMap(product, specMap),
  }));

  const matchingProducts = productValueMaps
    .filter(({ product, values }) => matchesFilterSet(product, values, selectedFilters, query))
    .map(({ product }) => product);
  ...
  if (brandCounts.size > 0) {
    const options = Array.from(brandCounts.entries())
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([value, count]) => ({
        value,
        label: value,
        count,
        selected: (selectedFilters.get("brand") ?? []).includes(value),
        enabled: true,
  ...
  const sortedSchema = [...filterSchema].sort((a, b) => {
  ...
  for (const filter of sortedSchema) {
    ...
    const derivedValues = Array.from(counts.keys())
      .filter((value) => !predefinedOptions.some((option) => option.value === value))
    ...
    const filterDependencies = getFilterDependencies(filter);

    const options = optionItems
      .map(({ option, value, label }) => {
        const dependencies = getOptionDependencies(option);
        ...
        if (count === 0 && !isSelected && optionItems.length > 0 && !(filter.isRange || filter.valueType === "BOOLEAN")) {
          return null;
        }
    ...
    filters.push({
      ...
      key: filter.id,
      label: filter.name,
      type: filter.isRange ? "checkbox" : filter.valueType === "BOOLEAN" ? "checkbox" : "checkbox",
      group: filter.filterGroup,
      order: filter.filterOrder,
      options,
      dependencies: filterDependencies.length > 0 ? filterDependencies : undefined,
    });
  }
