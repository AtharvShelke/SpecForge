...
// Guide Card
// -------------------------------------------------------------------
const GuideCard: FC<{ guide: BuildGuide; onStartBuild: () => void }> = ({
  guide,
  onStartBuild,
}) => {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div
        className={`bg-gradient-to-br ${theme.color} p-6 relative overflow-hidden`}
      >
        <div
        ...
        <div className="relative flex items-start justify-between">
          <div>
            <span
              className={`inline-block text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full mb-2 bg-white/20 text-white`}
            >
              {theme.tier}
            </span>
        ...
        <div className="relative mt-4 flex items-end justify-between">
          <div>
            ...
            <p className="text-white font-bold text-lg">
              ~₹{guide.total.toLocaleString("en-IN")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-5">
        <p className="text-sm text-zinc-600 leading-relaxed mb-4">
          {guide.description || "A hand-curated PC build."}
        </p>
        ...
        {expanded && (
          <div className="mb-4 space-y-2 animate-in slide-in-from-top-2 duration-200">
            {guide.items?.map((item, i) => (
              <div
                key={item.id || i}
                className="flex items-center justify-between text-xs px-3 py-2.5 bg-zinc-50 rounded-lg border border-zinc-100"
              >
                <div className="min-w-0 pr-4">
                  ...
                  <span className="text-zinc-800 font-medium truncate block">
                    {item.variant?.product?.name || "Unknown Component"}
                  </span>
                </div>
...
export default function BuildGuidesPage() {
  ...
  const filtered = buildGuides.filter((g) => {
    ...
    const max_f =
      BUDGET_FILTERS.find((f) => f.label === activeFilter)?.max ?? Infinity;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filtered.map((guide) => (
                <GuideCard
                  key={guide.id}
                  guide={guide}
                  onStartBuild={() => handleStartBuild(guide)}
                />
              ))}
            </div>
          </>
        ...
        <Section spacing="xl" container={false} className="mt-8">
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 sm:p-10 text-center">
            ...
            <p className="text-blue-100 text-sm mb-6 max-w-lg mx-auto">
              Use our Build Mode to configure any combination — our
              compatibility checker will flag issues in real time.
            </p>
            <Link
