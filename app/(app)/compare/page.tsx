      ...
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center p-4">
        <div className="text-center bg-white border border-zinc-200 rounded-2xl p-10 shadow-sm max-w-sm w-full">
          ...
          <p className="text-zinc-500 text-sm mb-6">
            You haven't added any items to compare yet.
          </p>
          <Link
            href="/products"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">
              {compareItems.length}{" "}
              {compareItems.length === 1 ? "item" : "items"} selected
            </p>
          </div>
        </div>
            ...
            <thead>
              <tr>
                ...
                {compareItems.map((item) => (
                  <th
                    key={item.id}
                    className="p-6 min-w-[250px] w-64 border-b border-l border-zinc-200 align-top relative"
                  >
                    <button
                    ...
                    >
                      ...
                      {item.variants?.[0]?.status !== "IN_STOCK"
                        ? "Out of Stock"
                        : "Add to Cart"}
                    </button>
                  </th>
                ...
                <tr key={key} className="hover:bg-zinc-50/50 transition-colors">
                  ...
                  {compareItems.map((item) => {
                    ...
                    return (
                      <td
                        key={`${item.id}-${key}`}
                        className="p-4 sm:px-6 text-sm text-zinc-600 border-l border-zinc-200"
                      >
                        {val ? (
                          String(val)
                        ) : (
        ...
        {compareItems.length < 2 && (
          <div className="mt-8 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl p-4 text-sm text-center flex flex-col items-center">
            <p>
              Add at least one more item to see a useful side-by-side
              comparison.
            </p>
            <Link
              href="/products"
              className="mt-2 font-semibold underline hover:text-blue-800"
            >
              Find more products
            </Link>
          </div>
