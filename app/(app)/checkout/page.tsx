...
// Extracted so the order list never re-renders when form state changes
const OrderSummary = memo(function OrderSummary({
    ...
    cart:         ReturnType<typeof useShop>['cart']
    subtotal:     number
    gstAmount:    number
    total:        number
            ...
            <div className="bg-gray-50 p-6 sm:p-8 border-t border-gray-100">
                ...
                >
                    {isSubmitting ? 'Processing...' : isAdmin ? (
                        <><ShieldAlert size={20} /> Admin Manual Reserve (POS)</>
                    ) : (
                        <><CreditCard size={20} /> Pay ₹{total.toLocaleString('en-IN')} (Mock)</>
                    )}
                </button>
                <p className="mt-4 text-xs text-center text-gray-500">
                    {isAdmin
                        ? 'This will bypass payment gateways and directly reserve inventory.'
...
export default function CheckoutPage() {
    ...
    // Read ?admin= once on mount — no dep needed
    useEffect(() => {
        setIsAdmin(new URLSearchParams(window.location.search).get('admin') === 'true');
    }, []);

    // Close cart drawer once on mount
    ...
    }, [formData, isAdmin, cart, clearCart]);
    ...
    const { subtotal, gstAmount, total } = financials;
                        ...
                        <form id="checkout-form" onSubmit={handleSubmit} className="bg-white shadow-sm rounded-2xl p-6 sm:p-8 border border-gray-100">
                            ...
                            <div className="grid grid-cols-1 gap-y-5 gap-x-4 sm:grid-cols-2">
                                <div className="sm:col-span-2">
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="customerName" id="customerName" required value={formData.customerName} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" placeholder="John Doe" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="email" name="email" id="email" required value={formData.email} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" placeholder="john@example.com" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="tel" name="phone" id="phone" required value={formData.phone} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" placeholder="+91 9876543210" minLength={14} maxLength={14} />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-y-5 gap-x-4 sm:grid-cols-2">
                                <div className="sm:col-span-2">
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="shippingStreet" id="shippingStreet" value={formData.shippingStreet} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="shippingCity" id="shippingCity" value={formData.shippingCity} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="shippingState" id="shippingState" value={formData.shippingState} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="shippingZip" id="shippingZip" value={formData.shippingZip} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <input type="text" name="shippingCountry" id="shippingCountry" value={formData.shippingCountry} onChange={handleInputChange} className="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm px-4 py-3 bg-gray-50" />
                                    </div>
                                </div>
                            </div>
                        </form>
