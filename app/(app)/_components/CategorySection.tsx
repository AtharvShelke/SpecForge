...
const FALLBACK_IMAGES = [
    ...
    'https://images.unsplash.com/photo-1603481586273-2f908a50c263?q=80&w=1200&auto=format&fit=crop',
]
...
interface CardProps {
    cat:      { category: string; label: string; image?: string | null; productCount: number }
    index:    number
    priority: boolean
}

const CategoryCard = memo(function CategoryCard({ cat, index, priority }: CardProps) {
    return (
        <motion.div
            ...
            <div className="relative z-10 sm:translate-y-4 sm:group-hover:translate-y-0 transition-transform duration-500">
                ...
                <div className="flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                    <p className="text-sm font-medium text-zinc-300">
                        {cat.productCount !== undefined ? `${cat.productCount} Premium Products` : 'Explore Hardware'}
                    </p>
...
export default function CategorySection() {
    ...
    useEffect(() => {
        fetch("/api/storefront/categories")
            ...
            .catch(err => console.error("Failed to fetch storefront categories:", err))
    }, [])
