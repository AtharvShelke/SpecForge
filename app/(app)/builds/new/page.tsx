...
function formatSpecs(product: Product) {
  ...
  const getSpec = (key: string) => {
    ...
    return spec?.value ? String(spec.value) : "";
  };

  const upperCat = String(categoryName).toUpperCase();
  if (upperCat.includes("PROCESSOR") || upperCat.includes("CPU")) {
    const socket = getSpec("socket") || getSpec("cpu-socket") || getSpec("Socket");
    const cores = getSpec("cores") || getSpec("coresThreads") || getSpec("Cores");
    const speed = getSpec("speed") || getSpec("clockSpeed") || getSpec("base_clock") || getSpec("Base Clock");
    const parts = [];
    if (socket) parts.push(socket);
    if (cores) parts.push(`${cores} Cores`);
    if (speed) parts.push(speed.endsWith("GHz") ? speed : `${speed} GHz`);
    return parts.join(" • ");
  }
  if (upperCat.includes("MOTHERBOARD") || upperCat.includes("MB")) {
    const socket = getSpec("socket") || getSpec("cpu-socket") || getSpec("Socket");
    const formFactor = getSpec("form_factor") || getSpec("formFactor") || getSpec("Form Factor");
    const ramType = getSpec("ddr_gen") || getSpec("memoryType") || getSpec("ramType") || getSpec("DDR Generation");
    const parts = [];
    if (socket) parts.push(socket);
    if (formFactor) parts.push(formFactor);
    if (ramType) parts.push(ramType);
    return parts.join(" • ");
  }
  if (upperCat.includes("RAM") || upperCat.includes("MEMORY")) {
    const type = getSpec("type") || getSpec("ramType") || getSpec("DDR Generation") || getSpec("Memory Type");
    const capacity = getSpec("capacity") || getSpec("Capacity");
    const speed = getSpec("speed") || getSpec("Speed");
    const parts = [];
  ...
  if (upperCat.includes("GRAPHICS") || upperCat.includes("GPU") || upperCat.includes("VIDEO")) {
    const chipset = getSpec("gpu_chip") || getSpec("chipset") || getSpec("GPU Chip");
    const memory = getSpec("vram") || getSpec("memorySize") || getSpec("VRAM");
    const parts = [];
    if (chipset) parts.push(chipset);
    if (memory) parts.push(memory);
    return parts.join(" • ");
  }
  if (upperCat.includes("STORAGE") || upperCat.includes("SSD") || upperCat.includes("HDD")) {
    const capacity = getSpec("capacity") || getSpec("size") || getSpec("Capacity");
    const type = getSpec("drive_type") || getSpec("type") || getSpec("Drive Type");
    const parts = [];
    if (capacity) parts.push(capacity);
    if (type) parts.push(type);
    return parts.join(" • ");
  ...
  if (upperCat.includes("POWER_SUPPLY") || upperCat.includes("PSU")) {
    const wattage = getSpec("wattage") || getSpec("power") || getSpec("Wattage");
    const efficiency = getSpec("efficiency") || getSpec("rating") || getSpec("Efficiency");
    const parts = [];
    if (wattage) parts.push(wattage.endsWith("W") ? wattage : `${wattage}W`);
    if (efficiency) parts.push(efficiency);
    return parts.join(" • ");
  }
  if (upperCat.includes("CASE") || upperCat.includes("CABINET")) {
    const form = getSpec("form_factor") || getSpec("formFactor") || getSpec("Form Factor");
    const color = getSpec("color") || getSpec("Color");
    const parts = [];
    if (form) parts.push(form);
    if (color) parts.push(color);
    return parts.join(" • ");
  }
  if (upperCat.includes("COOLER") || upperCat.includes("FAN")) {
    const type = getSpec("cooler_type") || getSpec("type") || getSpec("Cooler Type");
    const tdp = getSpec("tdp_rating") || getSpec("tdp") || getSpec("TDP Rating");
    const parts = [];
    if (type) parts.push(type);
    if (tdp) parts.push(tdp.endsWith("W") ? tdp : `${tdp}W`);
    return parts.join(" • ");
  }

  const brand = getSpec("brand") || getSpec("Brand");
  const color = getSpec("color") || getSpec("Color");
  const parts = [];
