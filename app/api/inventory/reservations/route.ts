...
import { createReservation, getReservations } from "@/lib/services/inventory.service";
import { ServiceError } from "@/lib/services/catalog.service";

export async function GET(req: NextRequest) {
  try {
    const searchParams = req.nextUrl.searchParams;
    const reservations = await getReservations(searchParams.get("orderId") || undefined);
